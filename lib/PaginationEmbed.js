...
class PaginationEmbed {
     ...
     * @property {Boolean} options.deleteEnabled Whether or not the delete button should be enabled. Defaults to extendedButtons value
     ...
     */
    constructor(bot, message, pages = [], options = {}, file) {
        this.bot        = bot;
        this.pages      = pages;
        ...
        this.deleteEnabled = (typeof options.deleteEnabled !== 'undefined') ? options.deleteEnabled : this.advanced;
    }
     ...
     */
    async initialize() {
        this.removePerm = this.invoker.channel.guild !== undefined && (await this.invoker.channel.permissionsOf(this.bot.user.id)).has('manageMessages');
        ...
        if (this.advanced) {
            ...
            if (this.removePerm && this.deleteEnabled) await this.message.addReaction(this.delete);
        } else {
            await this.message.addReaction(this.back);
            await this.message.addReaction(this.forth);
        }
    ...
    run() {
        this.handler.on('reacted', async event => {
            switch (event.emoji.name) {
                case this.firstPage: {
                    if (this.advanced) {
                        if (this.removePerm) await this.message.removeReaction(this.firstPage, this.invoker.author.id);
                ...
                case this.back: {
                    if (this.removePerm) await this.message.removeReaction(this.back, this.invoker.author.id);
                ...
                case this.forth: {
                    if (this.removePerm) await this.message.removeReaction(this.forth, this.invoker.author.id);
                ...
                case this.lastPage: {
                    if (this.advanced) {
                        if (this.removePerm) await this.message.removeReaction(this.lastPage, this.invoker.author.id);
                ...
                case this.delete: {
                    if (this.advanced && this.removePerm) await this.message.removeReactions();
        ...
        this.handler.on('end', async () => {
            if (this.removePerm) await this.message.removeReactions();
        });
    }
}

module.exports = {
    createPaginationEmbed: async (bot, message, pages, options, file) => {
        const paginationEmbed = new PaginationEmbed(bot, message, pages, options, file);
